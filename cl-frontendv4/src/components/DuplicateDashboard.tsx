...
interface EmailAndLinkedInScripts {
  email1: string;
  email2: string;
  email3: string;
  linkedIn1: string;
...
const emailScriptsTitle: Record<keyof EmailAndLinkedInScripts, string> = {
  email1: "Email PAS",
  email2: "Email AIDA",
  email3: "Email FAB",
...
const swrKey = "api/data";

const DuplicateDashboard = ({ records: data }: Props) => {
  const { data: records } = useSWR(swrKey, getBuyerData, { fallbackData: data });
  ...
  const [emailAndLinkedInScripts, setEmailAndLinkedInScripts] =
    useState<EmailAndLinkedInScripts>({} as EmailAndLinkedInScripts);
  ...
  useEffect(() => {
    mutate(swrKey); // Forces a re-fetch for the given key
  }, []);

  useEffect(() => {
    if (records && records.length > 0) {
      const emailScripts = records[currentIndex]?.emailScripts[0];
      setEmailAndLinkedInScripts(emailScripts);
    }
  ...
  const saveToDb = async (content: string, fieldName: string) => {
    let payload = {
      buyerIdentifier: records?.[currentIndex]?.buyer_identifier,
      emailScripts: { ...emailAndLinkedInScripts, [fieldName]: content },
    };
                  ...
                  <Select
                    ...
                    onSelect={(value) => {
                      if (value === "All") {
                        ...
                        setCurrentIndex(
                          records?.findIndex(
                            (record: any) => record.company === value
                          )
                        );
                      }
                    }}
  ...
  //     <>
  //       <CallerGuide
  //         caller_guide_text={callerGuideSectionText}
  ...
  const RenderEmailViewer = () => {
    return Object.keys(emailAndLinkedInScripts ?? {}).map((key) => {
      return (
        <div
          key={key}
          className="flex flex-col gap-4 items-center mt-[3%] ml-[5%] md:ml-[10%] mr-[5%] md:mr-[10%]"
        >
          <p className="text-2xl md:text-3xl font-semibold">
            {emailScriptsTitle[key as keyof EmailAndLinkedInScripts]}
          </p>
          <div className="h-[300px] max-h-[300px] w-full border-2 rounded shadow-md">
            <RichTextEditorModal
              initialText={
                emailAndLinkedInScripts[key as keyof EmailAndLinkedInScripts]
              }
              saveToDb={async (content) => saveToDb(content, key)}
              title={emailScriptsTitle[key as keyof EmailAndLinkedInScripts]}
            />
          </div>
        </div>
      );
