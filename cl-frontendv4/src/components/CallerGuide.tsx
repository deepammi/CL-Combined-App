...
const CallerGuide = ({ caller_guide_text, call_script }: any) => {
    ...
    const getConcatenatedBody = () => {
        ...
        const mergedString = "You are a caller’s assistant. You have to create a short Calling Script for calling a customer using the following Context\n\n" + checkedBodies.join("\n\n");
    ...
    async function handleSubmit() {
        try {
            ...
            setIsSubmitted(true);
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASEURL}/guide/caller-guide`, {
                method: "POST",
                headers: {
    ...
    return (
        <div className=" flex flex-col gap-4 items-center w-[100%] mt-10">
                ...
                {call_script.map((check: CallScriptItem) => {
                    ...
                    return (
                        <div
                            ...
                            className={`flex border-2 border-neutral-200 h-10 items-center justify-around gap-2 cursor-pointer rounded-full px-2 py-1 hover:bg-blue-600 overflow-hidden hover:text-white transition-all duration-500 w-7 sm:w-[calc(33.33%-1rem)] md:w-[calc(25%-1rem)] ${isChecked ? "bg-slate-200 rounded-full" : ""}`}
                        >
                            {isChecked && (
                                <>
            ...
            {receivedResponse ? (
                ...
                <div className="w-[80%] h-auto border-2 rounded shadow-md p-5">
                     <p>
                        AI generated caller guidance/assistance will appear here.
                     </p>
                 </div>
            )}
        </div>
